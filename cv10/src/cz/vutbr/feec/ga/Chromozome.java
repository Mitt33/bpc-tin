...
public class Chromozome {
	...
	public static final int POLY_LENGTH = (NUM_OF_POINTS * 2 + 3 + 1);
	...
	public void mutation(double pravdepodobnost) {
		
		Random rnd = new Random();
		for(int i = 0; i< NUM_OF_POLYG; i++) {
			if (rnd.nextDouble() < pravdepodobnost) {
				for (int j = 0; j < POLY_LENGTH ; j++) {
					data[POLY_LENGTH*i+j] = rnd.nextInt(256);
				}
			}
		}
	}
